...
import org.utt.app.DentalFrame;
import org.utt.app.InApp;
import org.utt.app.dao.DBmanager;
import org.utt.app.util.DateLabelFormatter;
import org.utt.app.util.Setup;

public class DentalPanel extends JPanel implements  Observer{
	ObjectOPD oUserInfo;
	Dimension screen;
	...
	public DentalPanel(ObjectOPD oUserInfo) {
		...
		dentalFormPanel = new DentalFormPanel(oUserInfo);
		mainHDFSOPDPanel =new MainHDFSOPDPanel(oUserInfo);

		dentalConf = new DentalConf();
	    ...
	    tabbedPane.addTab("<html>D<br>E<br>N<br>T<br>A<br>L </html>", null, split, "DENTAL");
	...
	public void setLeft(){
		...
		PtStatus2.setBounds(143, 5, 50, 25);
	...
	public void setRight(){
        ...
        tabbedPane.addTab("<html><b>FORM</b></html> " ,new ImageIcon(getClass().getClassLoader().getResource("images/bullet_black.png")),dentalFormPanel);
        tabbedPane.addTab("<html><b>EMR</b></html> ",new ImageIcon(getClass().getClassLoader().getResource("images/bullet_black.png")) ,mainHDFSOPDPanel);
       ...
       // tabbedPane.addTab("<html><b>EMR-tester</b></html> ",new ImageIcon(InApp.class.getResource("/org/utt/app/gfx/running.png")) ,mainHDFSOPDPanelTest);
        tabbedPane.addTab("<html><b>Dental Report Setting</b></html> ",new ImageIcon(getClass().getClassLoader().getResource("images/bullet_black.png")) ,dentalConf);
	}
	...
	public void searchVN(String vn,String hn,String clinic){
		...
		try {
			...
			for(int i=0;i<DentalFrame.hn_z515.length;i++){
				if(hn.equals(DentalFrame.hn_z515[i])){
					memo2="  Z515";
					break;
				}
				//System.out.println(i+1+". "+labunitname[i][0]);
	...
	public void searchPthn(String hn_in){
		...
		if(!Setup.checkEmptyTextField(TextField_HN) && TextField_HN.getText().trim().length()==7){
			...
			PreparedStatement stmt,stmt1;
			ResultSet rs,rs1;
			String query="select  vnmst.vn, vnmst.hn, vnpres.diagoutdatetime ,vnpres.clinic from vnmst,vnpres where vnmst.vn=vnpres.vn and vnmst.visitdate=vnpres.visitdate and vnmst.visitdate=?  "; 
			Vector<Vector<String>> data = new Vector<Vector<String>>();
			try {
				conn=new DBmanager().getConnMSSql();
				stmt = conn.prepareStatement(query);
